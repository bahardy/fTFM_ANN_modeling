...
class DataProcessor:
    ...
    @staticmethod
    def train_test_split_TBNN(inputs, tb, outputs, fraction=0.8, randomize=True, seed=None):
        """
        ...
        return inputs[train_idx, :], tb[train_idx, :, :], outputs[train_idx, :], \
               inputs[test_idx, :], tb[test_idx, :, :], outputs[test_idx, :], train_idx, test_idx
    
    def train_test_split(inputs, outputs, fraction=0.8, randomize=True, seed=None):
        """
        Split inputs and outputs into training and validation set
        :param inputs: input markers 
        :param outputs: outputs
        :param fraction: fraction to use for training data
        :param randomize: if True, randomly shuffles data along first axis before splitting it
        :return:
        ...
        return inputs[train_idx, :], outputs[train_idx, :], \
               inputs[test_idx, :], outputs[test_idx, :], train_idx, test_idx
